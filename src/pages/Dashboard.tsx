...
export default function Dashboard() {
  const { user, role, plan, remindersEnabled } = useFirebase();
    ...
    <div className="p-4 lg:p-6 pb-28 font-sans max-w-5xl mx-auto relative select-none">
      ...
      {missedCount > 0 && (
        <motion.div 
          ...
          className="bg-[#FEF5F5] border border-rose-200/50 p-4.5 rounded-[1.5rem] mb-8 flex items-center gap-3 shadow-sm"
        >
          <div className="text-rose-500 shrink-0">
            <AlertCircle size={22} />
          </div>
          <p className="text-sm text-neutral-800 font-normal">
            You have <span className="font-bold text-rose-600">{missedCount} missed follow-ups</span>
          </p>
        </motion.div>
      )}

      {/* 4 Clickable Today's Task Cards (Styled as in Screenshot) */}
      <div className="grid grid-cols-2 gap-4 mb-8 font-sans">
        {/* Today Follow-ups (Warm cream background with thin outline) */}
        <button 
          onClick={() => setActiveActionModalFilter("today")}
          className="bg-[#FFFDF0] border border-neutral-800 p-3.5 rounded-2xl shadow-sm hover:scale-[1.01] active:scale-95 transition-all text-neutral-800 flex items-center text-left gap-3 group w-full cursor-pointer outline-none"
        >
          <div className="w-10 h-10 bg-white text-amber-500 rounded-xl flex items-center justify-center shrink-0 border border-neutral-100 shadow-sm group-hover:scale-105 transition-transform duration-250">
            <Clock size={16} />
          </div>
          <div>
            <span className="text-[10px] uppercase font-bold tracking-wider text-neutral-500 block leading-none">Today Follow-ups</span>
            <span className="text-xl md:text-2xl font-black text-neutral-950 mt-1 block leading-none">{followupsDueCount}</span>
          </div>
        </button>

        {/* Missed Follow-ups (Soft warm red background) */}
        <button 
          onClick={() => setActiveActionModalFilter("missed")}
          className="bg-[#FEF5F5] border border-neutral-800 p-3.5 rounded-2xl shadow-sm hover:scale-[1.01] active:scale-95 transition-all text-neutral-800 flex items-center text-left gap-3 group w-full cursor-pointer outline-none"
        >
          <div className="w-10 h-10 bg-white text-rose-500 rounded-xl flex items-center justify-center shrink-0 border border-neutral-100 shadow-sm group-hover:scale-105 transition-transform duration-250">
            <AlertCircle size={16} />
          </div>
          <div>
            <span className="text-[10px] uppercase font-bold tracking-wider text-neutral-500 block leading-none">Missed Follow-ups</span>
            <span className="text-xl md:text-2xl font-black text-neutral-950 mt-1 block leading-none">{missedCount}</span>
          </div>
        </button>

        {/* Meetings Today (Warm cream background) */}
        <button 
        ...
        >
          ...
          <div>
            <span className="text-[10px] uppercase font-bold tracking-wider text-neutral-500 block leading-none">Meetings Today</span>
            <span className="text-xl md:text-2xl font-black text-neutral-950 mt-1 block leading-none">{meetingsCount}</span>
          </div>
        </button>

        {/* Closed Deals (Soft mint background) */}
        <button 
          onClick={() => setActiveActionModalFilter("closed")}
          className="bg-[#ECFBF4] border border-neutral-800 p-3.5 rounded-2xl shadow-sm hover:scale-[1.01] active:scale-95 transition-all text-neutral-800 flex items-center text-left gap-3 group w-full cursor-pointer outline-none"
        >
          <div className="w-10 h-10 bg-white text-emerald-500 rounded-xl flex items-center justify-center shrink-0 border border-neutral-100 shadow-sm group-hover:scale-105 transition-transform duration-250">
            <Trophy size={16} />
          </div>
          <div>
